...
export const CameraControlBox: FC<CameraControlBoxProps> = ({
  ...
  const camBox = useMemo(() => {
    ...
    const x =
      ((camera.x - bounds.left) / boundsDimensions.width) *
        boundBoxDimensions.width -
      boxWidth / 2;
    const y =
      ((camera.y - bounds.top) / boundsDimensions.height) *
        boundBoxDimensions.height -
      boxHeight / 2;
  ...
  const drawPaddingBox = useCallback((graphics: Graphics) => {
    graphics.clear();
  ...
  const drawBgBox = useCallback((graphics: Graphics) => {
    graphics.clear();
  ...
  const mouseMove = useCallback(
    (event: FederatedMouseEvent) => {
      if (activated) {
        const movement = {
          x: event.x - mouseInitPos.x,
          y: event.y - mouseInitPos.y,
        };
        const x =
          cameraInitPos.x +
          movement.x * (boundsDimensions.width / boundBoxDimensions.width);
        const y =
          cameraInitPos.y +
          movement.y * (boundsDimensions.height / boundBoxDimensions.height);
  ...
  return (
    <Container x={x} y={y}>
      <GraphicsComponent draw={drawPaddingBox} />
      <GraphicsComponent draw={drawBgBox} x={padding.left} y={padding.top} />
      <GraphicsComponent
        draw={drawCameraBox}
        x={padding.left}
        y={padding.top}
